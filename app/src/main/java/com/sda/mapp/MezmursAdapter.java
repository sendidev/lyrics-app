...
public class MezmursAdapter extends RecyclerView.Adapter<MezmursAdapter.Mezmurholder> {
 ...
 SharedPreferences.Editor editor;
   ...
   public void setfilterdlist(List<mezmurs> list){
    ...
    @Override
    public void onBindViewHolder(@NonNull Mezmurholder holder, int position1) {

        mezmurs ms= mymezmurs.get(position1);

        holder.id.setText(mymezmurs.get(position1).getId());
        holder.title.setText(mymezmurs.get(position1).getTitle());
        holder.aut.setText(mymezmurs.get(position1).getAut());
     ...
     //passing data from the recycler view item to on click listener
      ...
      TextView aut=holder.aut;
      ImageView likebut=holder.likebut;
      ConstraintLayout c2=holder.co2;
      CardView c1=holder.con1;
         listener.onItemcliklistener(mymezmurs.get(position1),c1,c2,id,title,likebut,position1);
            }
        });
              ...
              public void onClick(View view) {
                  ...
                  if (ms.getFavs().equals("0")) {
                      ...
                      fdb.insertIntoTheDatabase(ms.getTitle(),
                              ms.getId(), ms.getFavs(),ms.getAut(),ms.getSong());
                      likebut.setBackgroundResource(R.drawable.red_favorite_24);
                      Toast.makeText(con1.getContext(), ms.getTitle()+" ወደውታል", Toast.LENGTH_SHORT).show();
                  } else {
                      ms.setFavs("0");
                      fdb.removefav(ms.getId());
                      likebut.setBackgroundResource(R.drawable.baseline_favorite_border_24);

                      Toast.makeText(con1.getContext(), ms.getTitle()+" ከተወዳጅ ተወግዷል", Toast.LENGTH_SHORT).show();
                  }
